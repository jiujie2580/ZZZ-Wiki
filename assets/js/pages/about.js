...
(function () {
  ...
  async function init() {
    ...
    // ---------- 图片版权说明（v1.1.0 D5 / D7） ----------
    html += UI.section('图片版权说明',
      '<p class="disclaimer">本站为非官方《绝区零》粉丝百科，所有游戏角色、美术素材版权归 miHoYo / HoYoverse 所有。</p>' +
      '<ul class="about-list">' +
        '<li>图片仅用于资料整理与学习交流，不作商业用途。</li>' +
        '<li>禁止上传未公开测试素材、泄露剧情截图、付费内容截图。</li>' +
        '<li>如版权方认为某图片不妥，可通过仓库 Issue 联系下架。</li>' +
      '</ul>' +
      '<p class="disclaimer">网络不佳或版权敏感用户，可在网址后加 <code>?no-images=true</code> 关闭图片加载（也可通过本地设置持久关闭）。</p>'
    );
